...
- **Exception — saved signatures:** `GET /profile/signature/{user_id}` (`routers/profile.py`) is the one deliberately ownership-scoped endpoint in the app, since a saved signature can be used to auto-sign real documents on that user's behalf and so warrants tighter access than the general role-based rule above. It requires authentication (rejects an anonymous or invalid-token request with 401) and only serves the file to the signature's owner or an Admin/Superadmin (403 for anyone else) — verified with a real second user's login (not just the owner's), confirming both the 403 for a mismatched standard user and 200 once that same account is promoted to Admin. `{user_id}` is FastAPI-typed as `int`, and the served path is never built from client input (it's read back from `users.signature_path`, only ever written by `POST /profile/signature` from the authenticated uploader's own ID) — so there's no path-traversal surface here: a non-integer or slash-containing `user_id` is rejected by routing/validation before any file lookup runs. Avatars (`GET /profile/avatar/{user_id}`) are intentionally the opposite — public to any logged-in user, no ownership check — since they're meant to be visible wherever a user is referenced (header, user list, audit log) and carry none of a signature's risk.
