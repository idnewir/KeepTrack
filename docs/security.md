...
1. **JWT in `localStorage`.** Readable by any successful XSS. No XSS sink exists anywhere in this codebase today (checked directly), and the new strict CSP closes the most likely vector, but this mitigates the risk rather than removing the class of it. Moving to an httpOnly cookie would need CSRF protection, `SameSite` config, and every API call and the backend's own auth flow to change together — a genuine architecture change, not a line-level fix.
