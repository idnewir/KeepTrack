...
- **Dashboard mode switching:** `frontend/src/pages/DashboardPage.jsx` reads `isEnabled('debt_tracking')` once (`personalFinanceMode`) and branches its top metric row, its main chart panel (plain vs. collapsed-by-default "Show financial detail" toggle), and an extra `kt-panels-row` (debt summary / payments-due-this-month / savings goals panels) on that single flag — there is no separate "dashboard mode" setting anywhere; enabling the module *is* what switches the mode, and disabling it switches straight back. `backend/routers/dashboard.py`'s `GET /dashboard/summary` mirrors this: `DashboardSummary`'s debt fields (`total_debt`, `net_worth`, `debts_summary`, ...) are always present on the schema (default `None`/empty, same pattern as every other module-gated dashboard field) but only populated via `debt_service.build_dashboard_fields()` when `modules_service.is_enabled(db, "debt_tracking")` is true. See [decisions-log.md](decisions-log.md) for why this is automatic rather than a user-chosen preference.
- **Budget Planning module:** `backend/routers/budgets.py` and `backend/routers/savings_goals.py`, gated behind `require_module("budget_planning")`, cover annual category budgets (with optional monthly overrides) and savings goals — see [features.md](features.md#13-budget-planning) for the full feature set. Every budget figure (resolved per-month amount, actual spend, variance, year-to-date totals, status) is computed live by `backend/services/budget_service.py` from confirmed invoices, the same "compute, don't cache" approach as Planned Projects and Debt Tracking, never stored itself. Dashboard integration follows the same `isEnabled()`-gated, always-on-the-schema pattern as Debt Tracking: `GET /dashboard/summary` adds `budget_summary` and `savings_goals` fields (populated by `budget_service.build_dashboard_fields()`) only when `budget_planning` is enabled — a **Budget Planning panel** in organisational mode (`personalFinanceMode` false), and real data replacing the dashboard's Savings Goals placeholder in Personal Finance mode. A daily background thread (`services/budget_notification_service.py`, the same hand-rolled `threading.Thread` + sleep-loop pattern as every other background job in this app — not APScheduler, see [decisions-log.md](decisions-log.md)) checks each budgeted category's current-month spend against 80%/100% thresholds and notifies Admins, deduplicated via a `budget_notifications_sent` table.
