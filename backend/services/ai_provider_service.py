...
def generate_report_summary(report_data: dict, site_name: str, db: Session | None = None) -> dict:
    ...
    try:
        if cfg["provider"] == "anthropic":
            text = _call_anthropic(api_key, cfg["model"], _REPORT_SYSTEM_PROMPT, None, user_text)
        else:
            text = _dispatch_text_call(cfg["provider"], api_key, cfg["model"], cfg["endpoint_url"], _REPORT_SYSTEM_PROMPT, user_text)
    except Exception as exc:
