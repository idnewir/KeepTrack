...
def _cover_page(styles, title: str, site_name: str, date_from, date_to, generated_at: datetime, report_type: str) -> list:
    ...
    return [
        Spacer(1, 6 * cm),
        Paragraph(pdf_text(title), styles["cover_title"]),
        Paragraph(pdf_text(site_name), styles["cover_site"]),
        Spacer(1, 0.4 * cm),
        ...
        Paragraph(pdf_text(label.get(report_type, report_type.title())), styles["cover_meta"]),
        Spacer(1, 0.6 * cm),
...
def _summary_page(styles, ai_summary: dict) -> list:
    ...
    if insights:
        ...
        for insight in insights:
            flow.append(Paragraph(f"• {pdf_text(insight)}", styles["insight"]))
...
def _funding_table(styles, data: dict) -> Table | None:
    ...
    if funding["financial_year_label"]:
        entries.append(("Financial year", pdf_text(funding["financial_year_label"])))
...
def generate_report_pdf(
    ...
    story.append(Paragraph(f"Generated by {pdf_text(generated_by_username)}", styles["muted"]))
