...
export default function DashboardPage() {
  ...
  // 'This year' / 'Last year' chart shortcut (Part 2.5) — previousMonths is
  // fetched lazily the first time it's needed rather than alongside the main
  // summary, since most dashboard loads never touch it. See
  // docs/decisions-log.md.
  const [chartPeriod, setChartPeriod] = useState('current')
  const [previousMonths, setPreviousMonths] = useState(null)
  const [previousMonthsLoading, setPreviousMonthsLoading] = useState(false)

  const handleChartPeriodChange = useCallback(
    (nextPeriod) => {
      setChartPeriod(nextPeriod)
      if (nextPeriod === 'previous' && previousMonths === null && !previousMonthsLoading) {
        setPreviousMonthsLoading(true)
        dashboardApi
          .monthlyBreakdown('previous', token)
          .then((data) => setPreviousMonths(data.monthly_breakdown))
          .catch(() => setPreviousMonths([]))
          .finally(() => setPreviousMonthsLoading(false))
      }
    },
    [previousMonths, previousMonthsLoading, token]
  )
  ...
  const chartMonths = chartPeriod === 'current' ? summary.monthly_breakdown : previousMonths || []
            ...
            <div className={`kt-collapsible-chart${showFinancialDetail ? ' expanded' : ''}`}>
              <div>
                ...
                <p className="kt-panel-subtitle">
                  Click a line to see the detail behind it — income, actual spend, or the forecast. Use the
                  legend to show or hide any series.
                </p>
                <FinancialChart
                  months={chartMonths}
