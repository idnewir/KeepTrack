...
export default function PendingApprovalTab({ token }) {
  ...
  const handleReject = async (id) => {
    ...
    setBusyId(id)
    try {
      await authApi.rejectUser(id, token)
  ...
  return (
    <div>
      <p className="kt-panel-subtitle">Approve new registrations and assign their role, or reject them.</p>
            ...
            <div className="kt-settings-row" key={u.id}>
              ...
              <div className="kt-settings-row-control" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                {confirmingRejectId === u.id ? (
                  <>
                    <span className="kt-category-confirm-text">Reject this registration?</span>
                    <button
                      type="button"
                    ...
                    >
                      {busyId === u.id ? 'Rejecting…' : 'Yes, reject'}
                    </button>
                    <button
                      type="button"
